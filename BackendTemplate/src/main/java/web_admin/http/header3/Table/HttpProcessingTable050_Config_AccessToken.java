...
@Http(api = API.DBTable_Config_AccessToken)
@HttpDocument(id = 3050, Header = API.H3_Database, Description = "Tạo database đăng nhập thông thường")
public class HttpProcessingTable050_Config_AccessToken extends BaseHttpAdminWeb_SignedIn {
	public short DBId;
	...
	@Override public MyRespone onPOST(BackendSession session) {
		...
		try {
			...
			rf.seek(BGBaseDatabase.Offset_AccessKey);
			rf.writeLong(AccessKey);
